...
/* Named WString.h, not String.h, matching every other real Arduino core
 * (AVR, SAMD, etc.) -- found the hard way while building this: on a
 * case-insensitive filesystem (Windows/macOS, the default), a header
 * named String.h on the include path shadows the C standard library's
 * <string.h> for any #include <string.h>, breaking strlen/memcpy/etc.
 * project-wide the moment cores/nrf54l/ is on -I. */
